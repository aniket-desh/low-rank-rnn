#!/usr/bin/env bash
# Tier 3 — scale n. 27 runs:
#   n ∈ {64, 256, 1024} × β ∈ {0.2, 0.44, 0.6} × 3 seeds, lattice_2d.
# Parallelism degrades with n; we run 5 at n=64, 3 at n=256, 1 at n=1024.
set -euo pipefail
...
run_one() {
    ...
    local btag="${beta//./}"
    local save="$OUT_ROOT/tier3_lattice${n}_beta${btag}_seed${seed}"
...
# n=1024: 1 at a time (avoid memory pressure during diagnostics).
JOBS=()
for beta in 0.2 0.44 0.6; do
    for seed in 0 1 2; do
