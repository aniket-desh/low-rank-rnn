...
## 7. Blog / LessWrong figure suite

High-DPI publication figures generated by
`scripts/build_blog_figures.py --all` from the completed tiers, plus
animations rendered by `scripts/build_animations.py` from
`--save-every`-checkpointed runs.

### Static figures (`figures/blog/`)

| Figure | Source tier | Purpose |
|---|---|---|
| `main_result_triptych.png` | Tier 4 (Tier 6 if present) | Hero figure — r_eff, Δ, relative align vs β per n. |
| `effective_rank_heatmap.png` (+ raw, log) | Tier 4 / Tier 6 | Phase diagrams of learned operator dimension. |
| `scaling_exponent_alpha.png` (+ `.csv`) | Tier 4 / Tier 6 | Bootstrapped α(β) for r_eff ~ n^α, with 95% intervals. |
| `task_induced_coarse_graining.png` | Tier 5 | The "denoise halves r_eff with no Δ_baseline cost" panel. |
| `graph_family_comparison.png` | Tier 2 | Lattice / Curie / block phase diagrams. |
| `relative_alignment_phase_diagram.png` (+ A/C/Cτ panels) | Tier 4 / Tier 6 | Corrects raw alignment for the k/n random baseline. |
| `matrix_heatmaps_lattice64.png` | Tier 2/5/animation | Matrix decomposition A, C, C_τ, G at three β. |
| `singular_values_lattice64.png` | same | σᵢ(G) spectra at three β. |
| `top_modes_lattice64.png` | same | Top-4 left singular vectors reshaped 8×8. |

Each figure is committed in three formats: `.png` at 300 dpi, `.svg`, `.pdf`.

### Animations (`figures/blog/animations/`, MP4 + GIF; not committed by default)

| File | Content |
|---|---|
| `G_heatmap_beta0p{200,440,800}.mp4` | G = RJB evolving during training, one per temperature. |
| `singular_spectrum_three_betas.mp4` | σ(G) vs index, three β panels, over training. **Best single explanatory animation.** |
| `top_modes_beta0p440.mp4` | Top-4 modes at criticality over training. |
| `training_dashboard_three_betas.mp4` | r_eff/Δ/align/spectrum dashboard, three β. |

Regenerate with:

```bash
bash scripts/run_animation_sources.sh   # 5 checkpointed n=64 runs
